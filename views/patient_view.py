    ...
    if target_hn in patients_db['hn'].values:
        ...
        if not pt_visits.empty:
            ...
            if not valid_pefr_visits.empty:
                ...
                st.divider()
                st.subheader(f"ผลการประเมินล่าสุด")
                st.caption(f"ข้อมูลเมื่อ: {visit_date_str}")
                ...
                st.metric("ค่า PEFR ล่าสุด", f"{current_pefr} L/min")
                ...
                if "Yellow" in zone_name:
                    # ✅ เพิ่มข้อมูลยาพ่นฉุกเฉิน MDI ตรงนี้ (ตรวจสอบย่อหน้าแล้ว)
                    with st.expander("📢 วิธีใช้ยาพ่นฉุกเฉิน (MDI) ที่ถูกต้อง (คลิก)"):
                        st.markdown("""
                        **ขั้นตอนการพ่นยา:**
                        1. 🟢 **ถอดฝาครอบ** หลอดพ่นยาออก ตรวจดูสิ่งแปลกปลอม
                        2. 🔄 **เขย่าหลอด** ในแนวตั้งแรงๆ 3-4 ครั้ง
                        3. 😮‍💨 **หายใจออก** ทางปากให้สุด (ระวังอย่าพ่นลมหายใจใส่เครื่อง)
                        4. 👄 **อมปากหลอด** ให้สนิท (ระวังอย่าให้ลิ้นบังรูพ่นยา)
                        5. 🌬️ **กดหลอดพร้อมสูดหายใจเข้า** ทางปาก **ช้าๆ และลึกๆ** (กด 1 ครั้ง ต่อการสูด 1 ครั้ง)
                        6. ✋ **กลั้นหายใจ** ไว้ประมาณ 10 วินาที (เพื่อให้ยาค้างอยู่ในปอดนานที่สุด)
                        7. 😌 **ผ่อนลมหายใจ** ออกทางปากหรือจมูกช้าๆ
                        
                        ---
                        **⚠️ ข้อควรระวัง:**
                        * หากต้องพ่นซ้ำ ให้พักประมาณ **1 นาที** แล้วเริ่มทำใหม่ตั้งแต่ข้อ 2
                        * หากมียากระบอก (Spacer) ควรใช้ร่วมด้วยเพื่อประสิทธิภาพที่ดีขึ้น
                        """)
                        st.info("💡 ควรพกยาฉุกเฉินติดตัวตลอดเวลา และตรวจสอบวันหมดอายุเสมอ")
                ...
                # กราฟ (ส่งเฉพาะข้อมูลที่มีการเป่าจริงไปพล็อต)
                st.subheader("📈 กราฟแสดงค่าการเป่าปอด (Peak Flow)")
